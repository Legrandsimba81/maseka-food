...
interface Order {
  ...
  createdAt: string;
  items: { product: { name: string }; quantity: number; priceAtTime: number }[];
}
...
export default function TrackingPage() {
  ...
  const statusInfo = getDeliveryStatusLabel(order.deliveryStatus);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <div className="card p-4 space-y-4">
        ...
        <p><strong>Statut de livraison</strong> <span className={`px-2 py-1 rounded-full text-xs ${statusInfo.color}`}>{statusInfo.label}</span></p>

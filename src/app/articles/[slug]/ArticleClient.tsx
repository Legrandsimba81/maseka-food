...
export default function ArticleClient({ article, similarArticles }: { article: any; similarArticles: any[] }) {
  const imagesSecondary = Array.isArray(article.imagesSecondary) ? article.imagesSecondary : [];
  const articleUrl = typeof window !== 'undefined' ? window.location.href : `/articles/${article.slug}`;

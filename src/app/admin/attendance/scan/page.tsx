...
export default function ScanPage() {
  ...
  // Définition de onScanSuccess (sera appelée par le scanner)
  ...
  // Référence pour startScanner (pour l'appeler depuis onScanSuccess)
  const startScannerRef = useRef<() => Promise<void>>(async () => {});

  // Définition de startScanner (utilise onScanSuccessRef.current)
  ...
  // Mettre à jour les refs lorsque les fonctions changent
  useEffect(() => {
  ...
  useEffect(() => {
    startScanner();
    return () => {
      // le nettoyage est géré ci-dessus
    };
  }, [startScanner]);

...
import { LayoutDashboard, Package, ShoppingCart, Calendar, MessageSquare, Tag, ListOrdered } from "lucide-react";
          ...
          <div className="flex justify-center sm:justify-start space-x-4 sm:space-x-1 overflow-x-auto py-3">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center sm:gap-2 px-6 sm:px-4 py-2 lg:px-7 lg:py-2 text-sm font-medium transition-colors ${pathname === item.href
